...
// my code here
int select0(int a[], int lo, int hi, int k) {
    // Step 1: the original code specifies the base condition of lo >= hi,
    // as well as the base case of k == p.
    // my code first exchanges base recursive cases by inverting base
    //condition into lo < hi, retains the base case, and adds
    // additional conditional checks if the base case if not met.
    // That is, when k < p, and k > p.
    while (lo < hi){
        int p = partition(a, lo, hi);
        if (k == p){
            return a[k];
        ...
        if (k < p){
     // Step 2: Eliminate recursion by looping (while loop)
     // and reassign parameter values of lo and hi between iterations
            hi = p - 1;
        }
        else if (k > p){
     // Step 2 (cont'd).
    ...
    // The return value corresponds to Step 1's exchanging base recursive cases.
    // return a[lo] is originally a recursive base case,
    // that is, if lo >= hi, return a[lo]
    return a[lo];
}
